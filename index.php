        ...
        <div class="page">
            ...
            <form class="form" name="form_cambio_de_base" 
                ...
                <div class="input-seccion">
                    ...
                    <input id="numero" type="number" name="numero" value="<?= ($numero) ?? '' ?>" />
                    <span class="error <?= ($numeroRequeridoError ?? false) || ($numeroIncorrectoError ?? false) || ($digitoSuperaBaseError ?? false) ? 'error-visible' : '' ?>">
                        <?= ($numeroRequeridoError ?? false) ? NUMERO_REQUERIDO : '' ?>
                        <?= ($numeroIncorrectoError ?? false) ? NUMERO_INCORRECTO : '' ?>
                        <?= ($digitoSuperaBaseError ?? false) ? DIGITO_SUPERA_BASE : '' ?>
                    </span>
                </div>
                <div class="input-seccion">
                    <label for="baseorigen">Base Origen (2-16):</label> 
                    <input id="numero" type="number" name="baseorigen" value="<?= ($baseOrigen) ?? '' ?>" />
                    <span class="error <?= ($baseOrigenRequeridaError ?? false) || ($baseOrigenFueraLimiteError ?? false) ? 'error-visible' : '' ?>">
                        <?= ($baseOrigenRequeridaError ?? false) ? BASE_REQUERIDA : '' ?>
                        <?= ($baseOrigenFueraLimiteError ?? false) ? BASE_FUERA_LIMITE : '' ?>
                    </span>
                </div>
                <div class="input-seccion">
                    <label for="basedestino">Base Destino (2-16):</label> 
                    <input id="basedestino" type="number" name="basedestino" value="<?= ($baseDestino) ?? '' ?>" />
                    <span class="error <?= ($baseDestinoRequeridaError ?? false) || ($baseDestinoFueraLimiteError ?? false) ? 'error-visible' : '' ?>">
                        <?= ($baseDestinoRequeridaError ?? false) ? BASE_REQUERIDA : '' ?>
                        <?= ($baseDestinoFueraLimiteError ?? false) ? BASE_FUERA_LIMITE : '' ?>
                    </span>
                </div>
